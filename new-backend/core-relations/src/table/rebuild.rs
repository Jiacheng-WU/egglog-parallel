...
impl SortedWritesTable {
    pub(super) fn do_rewrite(
        ...
        if let Some(hint_col) = rewrite.hint_col() {
            ...
            // relevant.
            let to_scan = if let Some(last_version) = self.last_rebuilt_at.get(table_id) {
                if table.version().major == last_version.major {
        ...
        self.last_rebuilt_at.insert(table_id, table.version());
    }
